-- Fix: Remove ::text cast from record_id (UUID column expects UUID, not text)
...
BEGIN
  ...
  -- Registrar en audit_logs SIEMPRE (trazabilidad completa)
  INSERT INTO audit_logs (
    ...
    'TABLE_CLEARED',
    'table_states',
    v_table_state.id,  -- FIX: UUID directo, sin ::text
    jsonb_build_object(
    ...
    v_item_count > 0  -- Solo sensible si había items
  );
