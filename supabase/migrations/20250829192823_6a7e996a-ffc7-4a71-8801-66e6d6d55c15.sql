...
-- Policies
create policy "Users can view table states in their restaurant"
  on public.table_states for select
  using (
    restaurant_id in (select profiles.restaurant_id from profiles where profiles.id = auth.uid())
  );

create policy "Users can insert table states for their restaurant"
  on public.table_states for insert
  with check (
    restaurant_id in (select profiles.restaurant_id from profiles where profiles.id = auth.uid())
  );

create policy "Users can update table states in their restaurant"
  on public.table_states for update
  using (
    restaurant_id in (select profiles.restaurant_id from profiles where profiles.id = auth.uid())
  );

create policy "Users can delete table states in their restaurant"
...
create trigger update_table_states_updated_at
