...
export default function AliciaConfigRestrictions({ config, onSave }: Props) {
  ...
  const handleSave = async () => {
    ...
    // Preserve non-restriction rules
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-3">
            ...
            <label className="text-sm text-gray-700">¿Hay productos que NO vendes en domicilio?</label>
          </div>
          {noDeliveryProducts && (
            <Input value={noDeliveryText} onChange={e => setNoDeliveryText(e.target.value)} placeholder="Ej: Helados, sopas, platos especiales..." className="border-gray-200 ml-4" />
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-3">
            ...
            <label className="text-sm text-gray-700">¿Hay horarios donde ciertos productos no están disponibles?</label>
          </div>
          {timeRestrictions && (
            <Input value={timeText} onChange={e => setTimeText(e.target.value)} placeholder="Ej: Desayunos solo hasta las 11am, Almuerzos ejecutivos solo entre semana" className="border-gray-200 ml-4" />
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-3">
            ...
            <label className="text-sm text-gray-700">¿Hay tamaños o presentaciones que no manejas?</label>
          </div>
          {sizeRestrictions && (
            <Input value={sizeText} onChange={e => setSizeText(e.target.value)} placeholder="Ej: No hay tamaño familiar en pasta, las pizzas solo vienen en personal y mediana" className="border-gray-200 ml-4" />
          )}
        </div>

        {/* Other restrictions */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Otras restricciones</label>
          <div className="flex gap-2">
            <Input value={newRestriction} onChange={e => setNewRestriction(e.target.value)} placeholder="Ej: No vendemos alcohol después de las 11pm" onKeyDown={e => e.key === "Enter" && addOther()} className="border-gray-200" />
            <Button variant="outline" onClick={addOther} className="border-gray-200"><Plus className="h-4 w-4" /></Button>
          </div>
          {otherRestrictions.length > 0 && (
            <div className="mt-2 space-y-1.5">
              {otherRestrictions.map((r, i) => (
                <div key={i} className="flex items-center gap-2 text-sm bg-red-50 border border-red-100 p-2.5 rounded-lg">
                  <ShieldAlert className="h-3.5 w-3.5 text-red-400 shrink-0" />
                  <span className="flex-1 text-gray-700">{r}</span>
                  <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0 text-gray-400 hover:text-red-500" onClick={() => removeOther(i)}><Trash2 className="h-3 w-3" /></Button>
                </div>
              ))}
            </div>
          )}
