    ...
    <div className="bg-card border border-border/20 rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-5">
        ...
        {enabled && (
          <>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">¿Cuál es tu radio de domicilio?</label>
              <Input value={radius} onChange={e => setRadius(e.target.value)} placeholder="5 km / Toda la ciudad / Barrios cercanos" className="border-border" />
            </div>

            <div>
              <label className="block text-sm font-medium text-foreground mb-1">¿Cuánto cobras por domicilio? (si aplica)</label>
              <Input type="number" value={deliveryCost} onChange={e => setDeliveryCost(e.target.value)} placeholder="0 = gratis" className="border-border" />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">¿Qué le dice Alicia al cliente si la zona tiene cobro?</label>
