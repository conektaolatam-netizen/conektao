  ...
  return (
    <div className="bg-card border border-border/20 rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-5">
        <div className="bg-amber-900/30 border border-amber-500/30 rounded-lg p-3 flex gap-2">
        ...
        <div>
          ...
          <Input value={wabaId} onChange={e => setWabaId(e.target.value)} placeholder="1203273002014817" className="border-border" />
        </div>
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Email para recibir pedidos</label>
          <Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="pedidos@minegocio.com" className="border-border" />
        </div>
