...
export default function AliciaConfigPersonality({ config, onSave }: Props) {
  ...
  const handleSave = async () => {
    setSaving(true);
    // Preserve tagged rules from other sections
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">¿Cómo se llama tu asistente?</label>
          <Input value={assistantName} onChange={e => setAssistantName(e.target.value)} className="border-gray-200" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">¿Qué tono debe usar?</label>
        ...
        <div>
          ...
          <Textarea value={greeting} onChange={e => setGreeting(e.target.value)} rows={2} placeholder="¡Hola! Soy Alicia, ¿en qué te puedo ayudar?" className="border-gray-200" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Si el cliente necesita un humano, ¿a qué número lo envías?</label>
          <Input value={humanPhone} onChange={e => setHumanPhone(e.target.value)} placeholder="3001234567" className="border-gray-200" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Mensaje de escalamiento</label>
          <Input value={escMsg} onChange={e => setEscMsg(e.target.value)} placeholder="Comunícate al..." className="border-gray-200" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Reglas especiales del negocio</label>
          <p className="text-xs text-gray-400 mb-2">Instrucciones específicas que Alicia debe seguir</p>
          <div className="flex gap-2">
            <Input value={newRule} onChange={e => setNewRule(e.target.value)} placeholder="Ej: Si piden Camarones, preguntar: pizza, entrada o fettuccine?" onKeyDown={e => e.key === "Enter" && addRule()} className="border-gray-200" />
            <Button variant="outline" onClick={addRule} className="border-gray-200"><Plus className="h-4 w-4" /></Button>
          </div>
          <div className="mt-2 space-y-1.5 max-h-48 overflow-y-auto">
            {customRules.map((r, i) => (
              <div key={i} className="flex items-start gap-2 text-sm bg-gray-50 p-2.5 rounded-lg">
                <span className="flex-1 text-gray-700">{r}</span>
                <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0 text-gray-400 hover:text-red-500" onClick={() => removeRule(i)}><Trash2 className="h-3 w-3" /></Button>
              </div>
            ))}
          </div>
        </div>
