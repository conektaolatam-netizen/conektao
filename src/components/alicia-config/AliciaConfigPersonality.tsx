      ...
      <div className="p-5 space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-1 flex items-center gap-1.5">
            <Ban className="h-4 w-4 text-red-500" /> Palabras prohibidas adicionales
          </label>
          ...
          <div className="mt-2 space-y-1.5 max-h-32 overflow-y-auto">
            {prohibitedWords.map((w, i) => (
              <div key={i} className="flex items-center gap-2 text-sm bg-red-50 dark:bg-red-950/20 p-2 rounded-lg border border-red-200 dark:border-red-900/30">
                <span className="flex-1 text-foreground">"{w}"</span>
                <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0 text-muted-foreground hover:text-red-500" onClick={() => removeProhibited(i)}><Trash2 className="h-3 w-3" /></Button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-1 flex items-center gap-1.5">
            <ListChecks className="h-4 w-4 text-blue-500" /> Reglas de formato y estilo adicionales
          </label>
          <p className="text-xs text-muted-foreground mb-2">
          ...
          <div className="mt-2 space-y-1.5 max-h-32 overflow-y-auto">
            {rules.map((r, i) => (
              <div key={i} className="flex items-start gap-2 text-sm bg-blue-50 dark:bg-blue-950/20 p-2 rounded-lg border border-blue-200 dark:border-blue-900/30">
                <span className="flex-1 text-foreground">{r}</span>
                <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0 text-muted-foreground hover:text-red-500" onClick={() => removeFormatRule(i)}><Trash2 className="h-3 w-3" /></Button>
              </div>
            ))}
          </div>
        </div>
