...
import { Send, Sparkles, User, Bot, Loader2, TrendingDown, Building2, AlertTriangle, Globe } from 'lucide-react';
...
const suggestedQuestions = [
  { icon: TrendingDown, text: "¿Por qué bajaron las ventas esta semana?", color: "text-rose-400" },
  { icon: AlertTriangle, text: "¿Qué pasa con el Eje Cafetero?", color: "text-amber-400" },
  { icon: Building2, text: "Dame un resumen ejecutivo del día", color: "text-sky-400" },
  { icon: Globe, text: "¿Cuál es la región con mejor desempeño?", color: "text-emerald-400" },
];

const GeneralConektaoChat: React.FC = () => {
  ...
  const handleSend = async (messageText?: string) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/crepes-general-chat-ai`,
        {
          method: 'POST',
          headers: {
    ...
    <AIGlowBorder className="flex-1 flex flex-col h-full">
      <div className="bg-[#1a1a2e] flex flex-col h-full rounded-xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5">
          ...
          <div>
            <h2 className="text-base font-bold text-white">Conektao AI Ejecutivo</h2>
            <p className="text-xs text-white/40">Conectado a 42 sucursales · 8 regiones</p>
          </div>
        </div>
          ...
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-4">
              ...
              <p className="text-xs text-white/40 mb-6 max-w-sm">
                Pregúntame sobre cualquier región, sucursal o KPI. Tengo acceso a todos los datos de Crepes & Waffles.
              </p>
        ...
        <div className="p-4 border-t border-white/5">
          <div className="flex gap-2">
            <Textarea
              ...
              placeholder="Pregunta sobre todo Crepes & Waffles..."
...
export default GeneralConektaoChat;
