...
const Billing = () => {
  ...
  const loadCategoriesFromDB = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("categories").select("name").in("user_id", userIds).order("name");
      if (error) throw error;
  ...
  const categories = useMemo(() => {
    ...
    const allNames = Array.from(new Set([...(dbCategories || []), ...fromProducts])).filter(
      (name) => name && name.trim() !== "" && name.toLowerCase() !== "sin categoría",
    );
    const gradients = [
  ...
  const processRecipeConsumption = async (saleId: string) => {
    try {
      ...
      const { data: recipesData } = await supabase.from("recipes").select("name, ingredients").in("name", uniqueNames);
      const recipes: Array<{
        name: string;
        ingredients: any[];
      }> = (recipesData as any) || [];
  ...
  const confirmTableSelection = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("table_states").upsert(
        {
          restaurant_id: profile.restaurant_id,
    ...
    try {
      ...
      if (profile?.restaurant_id) {
        try {
          ...
          const { error: docError } = await supabase.from("business_documents").insert({
            restaurant_id: profile.restaurant_id,
            user_id: user?.id || "",
          ...
          <CardContent className="p-8">
            ...
            <div className="bg-green-50 rounded-lg p-4 mb-6">
              ...
              <p className="text-sm text-green-600">
                Pagado con {paymentMethods.find((m) => m.id === paymentMethod)?.name}
              </p>
            </div>
              ...
              <CardContent>
                <div className="space-y-3 mb-4">
                  {selectedProducts.map((product) => (
                    <div key={product.id} className="flex justify-between items-center p-3 bg-gray-700/30 rounded-lg">
                      ...
                      <span className="font-bold text-white">{formatCurrency(product.price * product.quantity)}</span>
                    </div>
                  ))}
                </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor="noTip" className="text-white">
                        No dio propina
                      </Label>
                    </div>
                      ...
                      <>
                        <div className="space-y-2">
                          <Label className="text-white">
                            Propina sugerida ({tipConfig.defaultTipPercentage}%):{" "}
                            {formatCurrency((calculateSubtotal() * tipConfig.defaultTipPercentage) / 100)}
                          </Label>
                        </div>

                        <div className="space-y-2">
                          <Label htmlFor="customTip" className="text-white">
                            Propina personalizada (opcional)
                          </Label>
                          <Input
                            id="customTip"
                            type="number"
            ...
            <Card className="bg-gray-800/50 border-white/10">
              ...
              <CardContent>
                  ...
                  {paymentMethods.map((method) => {
                    ...
                    return (
                      <Button
                        ...
                        className={`h-16 flex-col gap-2 ${paymentMethod === method.id ? `bg-gradient-to-r ${method.color} text-white border-none` : "border-white/20 text-white hover:bg-white/10"}`}
                ...
                {paymentMethod === "tarjeta" && (
                  <div className="bg-blue-900/30 p-4 rounded-lg space-y-3 border border-blue-500/30">
                    <h4 className="font-semibold text-blue-300 flex items-center gap-2">
                      <CreditCard className="h-4 w-4" />
                      Información de Tarjeta
                    </h4>
                    <div>
                      <Label htmlFor="voucher" className="text-white">
                        Código de Voucher
                      </Label>
                      <Input
                        id="voucher"
                ...
                {(paymentMethod === "transferencia" || paymentMethod === "daviplata" || paymentMethod === "nequi") && (
                  <div
                    className="bg-purple-900/30 p-4 rounded-lg space-y-4 border border-purple-500/30"
                    data-scroll-target="proof"
                  >
              ...
              <div className="relative overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  <div className="text-center">
                    ...
                    <p className="text-muted-foreground text-sm">Pregunta al cliente su correo electrónico</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      autoFocus
                    />
                    <p className="text-xs text-muted-foreground">✅ Se enviará la factura al correo del cliente</p>
                  </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        const newGuests = prompt(
                          `Número actual de comensales: ${currentGuests}\n\nIngresa el nuevo número:`,
                    ...
                    >
                      {tables.find((t) => t.number === selectedTable)?.guestCount ||
                        tables.find((t) => t.number === selectedTable)?.customers ||
                        1}{" "}
                      personas
                    </button>
                  </div>
                ...
                {selectedProducts.length > 0 &&
                  (kitchenOrderSent ? (
                    <div className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-green-100 to-emerald-100 border border-green-300 rounded-lg">
                      ...
                      <span className="text-green-600/70 text-xs hidden sm:inline">
                        · Agrega más productos para nueva comanda
                      </span>
                    </div>
                  ) : (
                    <Button
                      onClick={() => {
                        console.log(
                          "[BILLING] Abriendo panel inline cocina con productos:",
                          selectedProducts.length,
                          selectedProducts.map((p) => ({ name: p.name, qty: p.quantity })),
                        );
              ...
              {categories.map((category) => {
                ...
                return (
                  <Button
                    ...
                    className={`flex items-center gap-2 whitespace-nowrap ${selectedCategory === category.name ? `bg-gradient-to-r ${category.color} text-white border-none` : ""}`}
                  >
              ...
              {filteredProducts.map((product) => (
                ...
                >
                  ...
                  <CardContent className="p-4">
                    ...
                    {(profile?.role === "owner" ||
                      profile?.role === "admin" ||
                      profile?.permissions?.manage_products) && (
        ...
        <AlertDialog open={showClearTableModal} onOpenChange={(open) => !open && closeClearTableModal()}>
          <AlertDialogContent
            className={`max-w-md ${(clearTableData?.products.length ?? 0) > 0 ? "border-red-200 bg-gradient-to-b from-red-50 to-white" : "border-cyan-200 bg-gradient-to-b from-cyan-50 to-white"}`}
          >
            <AlertDialogHeader>
              <div className="flex items-center gap-3 mb-2">
                <div
                  className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg ${(clearTableData?.products.length ?? 0) > 0 ? "bg-gradient-to-br from-red-500 to-red-700 animate-pulse" : "bg-gradient-to-br from-cyan-500 to-cyan-700"}`}
                >
                ...
                <div>
                  <AlertDialogTitle
                    className={`text-xl font-bold ${(clearTableData?.products.length ?? 0) > 0 ? "text-red-700" : "text-cyan-700"}`}
                  >
                    {(clearTableData?.products.length ?? 0) > 0
                      ? `⚠️ Anular mesa ${clearTableData?.tableNumber}`
                      : `Liberar mesa ${clearTableData?.tableNumber}`}
                  </AlertDialogTitle>
                  <p
                    className={`text-xs font-medium ${(clearTableData?.products.length ?? 0) > 0 ? "text-red-500" : "text-cyan-500"}`}
                  >
                    {(clearTableData?.products.length ?? 0) > 0
                      ? "Acción crítica con auditoría"
                      : "Marcar mesa como disponible"}
                  </p>
                </div>
              </div>
              <AlertDialogDescription
                className={`text-base p-3 rounded-lg border ${(clearTableData?.products.length ?? 0) > 0 ? "bg-red-100 border-red-200" : "bg-cyan-100 border-cyan-200"}`}
              >
                {(clearTableData?.products.length ?? 0) > 0 ? (
                  <>
            ...
            <div className="my-4 space-y-4">
              ...
              <div
                className={`p-4 rounded-lg border ${(clearTableData?.products.length ?? 0) > 0 ? "bg-red-50 border-red-200" : "bg-cyan-50 border-cyan-200"}`}
              >
                <div className="flex items-center justify-between mb-2">
                  <span
                    className={`text-sm font-medium ${(clearTableData?.products.length ?? 0) > 0 ? "text-red-700" : "text-cyan-700"}`}
                  >
                    {(clearTableData?.products.length ?? 0) > 0 ? "Orden a anular:" : "Estado actual:"}
                  </span>
                  <Badge
                    variant={(clearTableData?.products.length ?? 0) > 0 ? "destructive" : "secondary"}
                    className="text-xs"
                  >
                ...
                <div
                  className={`space-y-1 text-sm ${(clearTableData?.products.length ?? 0) > 0 ? "text-red-600" : "text-cyan-600"}`}
                >
                  <p>
                    • <strong>{clearTableData?.products.length ?? 0}</strong> productos
                  </p>
                  <p>
                    • Total: <strong>{formatCurrency(clearTableData?.total ?? 0)}</strong>
                  </p>
                </div>
                {(clearTableData?.products.length ?? 0) > 0 && (
            ...
            <AlertDialogFooter
              className={`flex-col sm:flex-row gap-2 pt-4 border-t ${(clearTableData?.products.length ?? 0) > 0 ? "border-red-200" : "border-cyan-200"}`}
            >
              <AlertDialogCancel
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {tables.map((table) => (
                <Button
                  ...
                  className={`h-24 flex-col gap-2 transition-all duration-500 rounded-xl hover:scale-105 bg-gradient-to-br from-gray-900 via-gray-950 to-black relative overflow-hidden group ${
                    table.status === "libre"
                      ...
                      : "border-2 border-orange-500/90 shadow-[0_0_15px_rgba(249,115,22,0.4),inset_0_1px_0_rgba(255,255,255,0.1)] hover:shadow-[0_0_25px_rgba(249,115,22,0.6),0_0_50px_rgba(239,68,68,0.25),inset_0_1px_0_rgba(255,255,255,0.15)] hover:border-orange-400"
                  }`}
                >
                  {/* Efecto de brillo metálico */}
                  <div
                    className={`absolute inset-0 rounded-xl opacity-30 group-hover:opacity-50 transition-opacity duration-500 ${
                      table.status === "libre"
                        ? "bg-gradient-to-br from-green-400/20 via-transparent to-green-500/10"
                        : "bg-gradient-to-br from-orange-500/25 via-red-500/10 to-orange-600/15"
                    }`}
                  />
                  ...
                  <div
                    className={`absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 ${
                      table.status === "libre"
                        ...
                        : "bg-[radial-gradient(ellipse_at_center,rgba(249,115,22,0.2)_0%,rgba(239,68,68,0.1)_50%,transparent_70%)]"
                    }`}
                  />

                  <Utensils
                    className={`h-5 w-5 relative z-10 ${table.status === "libre" ? "text-green-400 drop-shadow-[0_0_6px_rgba(34,197,94,0.5)]" : "text-orange-400 drop-shadow-[0_0_6px_rgba(249,115,22,0.6)]"}`}
                  />

                  <div className="text-center relative z-10">
                    <div
                      className={`font-bold text-sm ${table.status === "libre" ? "text-green-300 drop-shadow-[0_0_8px_rgba(34,197,94,0.4)]" : "text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-400 drop-shadow-[0_0_8px_rgba(249,115,22,0.5)]"}`}
                    >
                      Orden {table.number}
                    </div>
                    {table.status === "libre" ? (
                      <div className="text-xs text-green-300/90 font-medium">Disponible</div>
                    ) : (
                      <div className="text-xs text-orange-200/90 space-y-1">
                        <div>{table.customers} personas</div>
                        {table.orderTotal > 0 && (
                          <div className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-300 to-red-400 drop-shadow-[0_0_6px_rgba(249,115,22,0.4)]">
        ...
        <AlertDialogContent className="max-w-md">
          ...
          <div className="my-4 p-4 bg-muted/50 rounded-lg border">
            <p className="text-sm text-muted-foreground">
              <strong>Orden {selectedTable}</strong> • {selectedProducts.length} productos •{" "}
              {formatCurrency(selectedProducts.reduce((sum, p) => sum + p.price * (p.quantity || 1), 0))}
            </p>
          </div>
