...
export default function AliciaConfigPage() {
  ...
  async function handleGenerateAlicia() {
    setGenerating(true);
    await saveMultipleFields({ is_active: true, setup_completed: true });
    setGenerating(false);
      ...
      <div className="bg-gray-50 border-b border-gray-100 px-4 py-3">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs text-gray-500 font-medium">Progreso de configuración</span>
            <span className="text-xs font-semibold text-teal-600">{completedCount}/{SECTIONS.length} secciones</span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-all text-left ${
                    active 
                      ? "bg-gradient-to-r from-teal-50 to-orange-50 text-gray-900 font-medium shadow-sm border border-teal-100" 
                      : "text-gray-600 hover:bg-gray-50"
                  }`}
                >
                  <div className={`flex items-center justify-center h-6 w-6 rounded-full shrink-0 ${
                    done ? "bg-green-500 text-white" : active ? "bg-teal-100 text-teal-600" : "bg-gray-100 text-gray-400"
                  }`}>
